...
export default function Login({ onLogin, onSwitchToSignup, theme }) {
  ...
  const s = {
    ...
    button: { background: t.primary, color: '#fff', border: 'none', padding: '16px', borderRadius: 16, fontSize: 16, fontWeight: 800, cursor: 'pointer', marginTop: 12, display: 'flex', justifyContent: 'center', alignItems: 'center' },
  ...
  return (
    <div style={s.container}>
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} style={s.card}>
        ...
        <p style={s.subtitle}>Elevate your experience with ShopAI premium features.</p>
        ...
        <div style={s.switch}>
          Don't have an account? <button onClick={onSwitchToSignup} style={s.link}>Create Account</button>
        </div>
      </motion.div>
    </div>
  );
